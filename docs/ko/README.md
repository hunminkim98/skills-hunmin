...
| 관련 웹 각도를 병렬로 검색하고 근거 있는 브리프 받기 | [deep-research](deep-research.md) |
| 의장이 웹 / 레포 / 결정과 순서를 고르게 하기 | [ultra-research](ultra-research.md) |

이 일들을 서로 바꿔 쓰지 마세요. 이 레포 안의 사실 조사와 넓은 웹 조사는 다릅니다. 토론은 작업 일지가 아닙니다. 그 셋을 내가 고르기 싫을 때 의장이 [ultra-research](ultra-research.md)입니다. 이 저장소에는 논문·아티클을 `raw/`에 쌓는 Karpathy식 위키가 없습니다. 챗봇 문체를 지우는 일은 따로 합니다. 옛 렌더를 격리하는 일은 죽은 코드를 지우는 일이 아닙니다.
...
cursor/
  ...
  output-debt/
  deep-research/
  ultra-research/     # 옆에 deep-research, deep-analyze, discussion이 있어야 함
```

`meta-prompting`은 형제 `prompt-optimizer`를 읽습니다. `ultra-research`는 조사·결정 형제 셋을 읽습니다. 그 묶음은 같이 설치하세요. 나머지는 혼자 동작합니다.
